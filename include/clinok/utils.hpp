...
// Returns a range over the command-line arguments, skipping argv[0].
//
// argv[0] is conventionally the program name and is excluded.
// argc < 1 is handled explicitly: while unusual, POSIX allows calling
// execve(2) with an empty argv, in which case argv[0] may be null.
...
1) Reorder is needed.
   ...
   Raw `res` here is:
     ["", "tail 1", "tail 2", ""]
   Origin order:
     [foo, ADD_HELP("tail 1"), ADD_HELP("tail 2"), help]
   which means: option `foo`, then trailing help texts, then autogenerated `--help`.
   We want `--help` before that trailing block, so it becomes:
     ["", "", "tail 1", "tail 2"]

2) Reorder is not needed.
   cli.def:
     STRING(foo, "Foo")
     ALIAS(f, foo)
     ADD_HELP("after aliases")

   Raw `res` is still something like:
     ["", "", "", "after aliases"]
   Origin order:
     [foo, help, alias f, ADD_HELP("after aliases")]
*/
template <size_t N>
constexpr auto reorder_help_messages(std::array<std::string_view, N> res, bool has_aliases) {
  // If `.def` ends with ADD_HELP(...), those lines should be printed after autogenerated --help.
  // Without aliases the generated order is: options, trailing help text, --help, so we move the
  // final empty slot (`--help`) before the trailing non-empty block. With aliases no reorder is needed.
  if (res.empty() || has_aliases)
    return res;

  for (auto i = res.size() - 1; i > 0 && !res[i - 1].empty(); --i)
